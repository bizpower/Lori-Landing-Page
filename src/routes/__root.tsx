import { Outlet, Link, createRootRoute, HeadContent, Scripts } from "@tanstack/react-router";

import appCss from "../styles.css?url";
import { Toaster } from "@/components/ui/sonner";

function NotFoundComponent() {
  return (
    <div className="flex min-h-screen items-center justify-center bg-background px-4">
      <div className="max-w-md text-center">
        <h1 className="text-7xl font-bold text-foreground">404</h1>
        <h2 className="mt-4 text-xl font-semibold text-foreground">Page not found</h2>
        <p className="mt-2 text-sm text-muted-foreground">
          The page you're looking for doesn't exist or has been moved.
        </p>
        <div className="mt-6">
          <Link
            to="/"
            className="inline-flex items-center justify-center rounded-md bg-primary px-4 py-2 text-sm font-medium text-primary-foreground transition-colors hover:bg-primary/90"
          >
            Go home
          </Link>
        </div>
      </div>
    </div>
  );
}

export const Route = createRootRoute({
  head: () => ({
    meta: [
      { charSet: "utf-8" },
      { name: "viewport", content: "width=device-width, initial-scale=1, viewport-fit=cover" },
      { title: "LORI CRM — Il CRM operativo per la Lead Generation" },
      { name: "description", content: "Stanco di utilizzare excel per la tua Lead generation ma i tool presenti sul mercato sono troppo complessi?" },
      { name: "author", content: "LORI CRM" },
      { name: "robots", content: "index,follow" },
      { property: "og:site_name", content: "LORI CRM" },
      { property: "og:locale", content: "it_IT" },
      { property: "og:type", content: "website" },
      { property: "og:title", content: "LORI CRM — Il CRM operativo per la Lead Generation" },
      { property: "og:description", content: "Stanco di utilizzare excel per la tua Lead generation ma i tool presenti sul mercato sono troppo complessi?" },
      { name: "twitter:card", content: "summary_large_image" },
    ],
    links: [
      { rel: "stylesheet", href: appCss },
      { rel: "icon", type: "image/svg+xml", href: "/favicon.svg" },
      { rel: "preconnect", href: "https://res.cloudinary.com", crossOrigin: "" },
      { rel: "dns-prefetch", href: "https://res.cloudinary.com" },
    ],
    scripts: [
      {
        type: "application/ld+json",
        children: JSON.stringify({
          "@context": "https://schema.org",
          "@graph": [
            {
              "@type": "Organization",
              "@id": "https://www.lori-crm.it/#organization",
              name: "LORI CRM",
              url: "https://www.lori-crm.it/",
              logo: "https://www.lori-crm.it/favicon.svg",
            },
            {
              "@type": "WebSite",
              "@id": "https://www.lori-crm.it/#website",
              url: "https://www.lori-crm.it/",
              name: "LORI CRM",
              inLanguage: "it-IT",
              publisher: { "@id": "https://www.lori-crm.it/#organization" },
            },
          ],
        }),
      },
    ],
  }),
  shellComponent: RootShell,
  component: RootComponent,
  notFoundComponent: NotFoundComponent,
});

function RootShell({ children }: { children: React.ReactNode }) {
  return (
    <html lang="it">
      <head>
        <HeadContent />
      </head>
      <body>
        {children}
        <Scripts />
      </body>
    </html>
  );
}

function RootComponent() {
  return (
    <>
      <Outlet />
      <Toaster />
    </>
  );
}
